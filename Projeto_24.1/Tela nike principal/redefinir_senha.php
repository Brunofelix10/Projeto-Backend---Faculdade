...
<head>
    ...
    <title>Nike - Redefinir senha</title>
</head>
<body>
...
<div class="wrapper">
    ...
    <form action="" name="redefinirform" method="post" onsubmit="return validarRedefinirSenha()">
        <h2 class="title">Redefinir Senha</h2>
        <input type="text" name="cpf" id="cpf" required placeholder="Cpf"><br>    
        <input type="password" name="nova_senha" id="nova_senha" minlength="8" maxlength="8" required placeholder="Nova senha"><br>
        <input type="password" name="confirmar_senha" id="confirmar_senha" minlength="8" maxlength="8" required placeholder="Confirme a nova senha"><br>
...
<?php
    ...
    if($_SERVER['REQUEST_METHOD']=="POST"){
        ...
        if ($resultado->num_rows == 1) {
            ...
            $sql_update = "UPDATE tb_usuario SET senha = ? WHERE cpf = ?";
            $stmt_update = $conexao->prepare($sql_update);
            if ($stmt_update === false) {
                die('Erro na preparação da consulta de atualização: ' . htmlspecialchars($conexao->error));
            }
            $stmt_update->bind_param('ss', $senha, $cpf);
            $stmt_update->execute();
